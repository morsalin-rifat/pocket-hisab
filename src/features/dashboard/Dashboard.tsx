...
import { Sidebar } from '../../components/Sidebar'; // মেনু ইম্পোর্ট
...
const Dashboard = ({ user }: { user: any }) => {
  ...
  const monthlyNormalExpenses = transactions.filter(t => t.type === 'expense' && !t.eventId && t.date.toDate() >= new Date(new Date().getFullYear(), new Date().getMonth(), 1)).reduce((sum, t) => sum + t.amount, 0);
    ...
    <div className="relative h-full w-full bg-black flex flex-col overflow-hidden">
      ...
      <div className="px-8 pt-12 pb-4 flex items-center justify-between z-30">
        <button onClick={() => setIsSidebarOpen(true)} className="w-12 h-12 rounded-2xl bg-white/5 border border-white/10 flex flex-col items-center justify-center gap-1.5 active:scale-90 transition-all">
          ...
          <div className="w-3 h-0.5 bg-white/50 self-start ml-3.5" />
          <div className="w-5 h-0.5 bg-white" />
        </button>
        
        <div className="text-center">
          <p className="text-[9px] font-black text-white/30 uppercase tracking-[4px]">Universe Balance</p>
          <h1 className="text-lg font-black text-white">{totalAssets.toLocaleString()} ৳</h1>
        </div>
      ...
      <div className="flex-1 overflow-y-auto no-scrollbar px-8 pb-48 pt-4">
        <LiquidBalance percentage={(remainingBudget / budget) * 100} amount={remainingBudget.toLocaleString()} label={activeEvent ? "Locked Mode" : "Budget Limit"} />
            ...
            {transactions.slice(0, 5).map((t) => (
              <div key={t.id} className="p-6 bg-white/[0.03] border border-white/5 rounded-[35px] flex justify-between items-center shadow-xl">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <h4 className="text-[10px] font-black text-white uppercase tracking-wider">{t.type === 'income' ? 'Income' : (t.category?.split(' ').slice(1).join(' ') || t.category)}</h4>
      ...
      <div className="absolute bottom-12 left-1/2 -translate-x-1/2 w-[90%] z-40">
        <div className="bg-[#0f0f0f]/95 backdrop-blur-3xl p-3 rounded-[40px] flex items-center justify-between border border-white/10 shadow-[0_25px_60px_rgba(0,0,0,1)]">
          ...
          <motion.button onClick={() => setIsMagicOpen(true)} whileTap={{ scale: 0.9 }} className="w-16 h-16 bg-white text-black rounded-[28px] flex items-center justify-center -mt-14 border-[6px] border-black text-3xl shadow-2xl">✨</motion.button>
