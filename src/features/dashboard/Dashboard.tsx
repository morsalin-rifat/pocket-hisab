  ...
  return (
    <div className="relative h-full w-full bg-black flex flex-col overflow-hidden font-sans">
      ...
      <div className="px-8 pt-12 pb-4 flex items-center justify-between z-30">
        <button onClick={() => setIsSidebarOpen(true)} className="w-12 h-12 rounded-2xl bg-white/5 border border-white/10 flex flex-col items-center justify-center gap-1.5 active:scale-90 transition-all"><div className="w-5 h-0.5 bg-white" /><div className="w-3 h-0.5 bg-white" /><div className="w-5 h-0.5 bg-white" /></button>
        <div className="text-center"><p className="text-[9px] font-black text-white/40 uppercase tracking-[4px]">Universe Assets</p><h1 className="text-2xl font-black text-white">{totalAssets.toLocaleString()} ৳</h1></div>
      ...
      <div className="flex-1 overflow-y-auto no-scrollbar px-8 pb-48 pt-4">
        <LiquidBalance percentage={(Math.max(0, budget - monthlyExpenses) / budget) * 100} amount={(budget - monthlyExpenses).toLocaleString()} label="Fuel Left" />
        <WeeklyReport transactions={transactions} />
        <SwipableCards walletBalances={balances} />
        <div className="mt-12 space-y-4">
          <h3 className="text-[10px] font-black uppercase tracking-[4px] text-white/30 text-center mb-6">Live Feed</h3>
          {transactions.slice(0, 5).map((t) => (
            <div key={t.id} className="p-6 bg-white/[0.03] border border-white/10 rounded-[35px] flex justify-between items-center shadow-lg"><div className="flex items-center gap-4"><div className="w-10 h-10 bg-white/5 rounded-xl flex items-center justify-center text-lg">{t.type === 'income' ? '💎' : '💸'}</div><div><h4 className="text-[11px] font-black text-white uppercase">{t.category}</h4><p className="text-[9px] text-white/40 font-bold">{t.note}</p></div></div><p className={`text-sm font-black ${t.type === 'income' ? 'text-emerald-400' : 'text-white'}`}>{t.type === 'income' ? `+${t.amount}` : `-${t.amount}`} ৳</p></div>
          ))}
        </div>
      </div>
      <div className="absolute bottom-12 left-1/2 -translate-x-1/2 w-[90%] z-40">
        <div className="bg-[#0c0c0c]/90 backdrop-blur-3xl p-3 rounded-[45px] flex items-center justify-between border border-white/10 shadow-[0_30px_70px_rgba(0,0,0,1)]">
