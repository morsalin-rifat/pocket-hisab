...
const Dashboard = ({ user }: { user: any }) => {
  ...
  return (
    <div className="relative h-full w-full bg-black flex flex-col overflow-hidden selection:bg-white/20 font-sans antialiased">
      ...
      <div className="px-8 pt-12 pb-4 flex items-center justify-between z-30">
        ...
        <div className="text-center">
          <p className="text-[10px] font-black text-white/40 uppercase tracking-[4px]">Total Capital</p>
          <h1 className="text-2xl font-black text-white tracking-tighter drop-shadow-2xl">{totalAssets.toLocaleString()} ৳</h1>
        </div>
        <div className="w-12 h-12 rounded-2xl bg-white border border-white/20 flex items-center justify-center font-black text-black text-sm shadow-xl" onClick={() => setIsBudgetOpen(true)}>
          {user.displayName?.[0]}
        </div>
      </div>

      <div className="flex-1 overflow-y-auto no-scrollbar px-8 pb-48 pt-4">
        <LiquidBalance percentage={(Math.max(0, budget - monthlyExpenses) / budget) * 100} amount={(budget - monthlyExpenses).toLocaleString()} label="Budget Limit" />
        <WeeklyReport transactions={transactions} />
        <SwipableCards walletBalances={balances} />
        
        <div className="mt-14 pb-10">
          <h3 className="text-[11px] font-black uppercase tracking-[6px] text-white/30 text-center mb-8">Recent Flow</h3>
            ...
            {transactions.slice(0, 5).map((t) => (
              <div key={t.id} className="p-6 bg-white/[0.03] border border-white/10 rounded-[35px] flex justify-between items-center shadow-lg">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <h4 className="text-[11px] font-black text-white uppercase tracking-wider">{t.category}</h4>
                    <p className="text-[10px] text-white/60 font-medium line-clamp-1">{t.note}</p>
                  </div>
                </div>
      ...
      {/* ২. প্রিমিয়াম এআই বাটন ও ডক */}
      <div className="absolute bottom-12 left-1/2 -translate-x-1/2 w-[90%] z-40">
        <div className="bg-[#0c0c0c]/90 backdrop-blur-3xl p-3 rounded-[45px] flex items-center justify-between border border-white/10 shadow-[0_20px_60px_rgba(0,0,0,1)]">
          <button onClick={() => setIsManualOpen(true)} className="flex-1 py-3 text-[10px] font-black text-white/40 uppercase text-center active:scale-90 transition-all tracking-widest">Manual</button>
          
          <motion.button 
            onClick={() => setIsMagicOpen(true)}
            whileHover={{ scale: 1.1 }}
            whileTap={{ scale: 0.9 }}
            className="w-16 h-16 bg-white rounded-[24px] flex items-center justify-center -mt-14 border-[6px] border-black shadow-[0_10px_30px_rgba(255,255,255,0.2)] relative overflow-hidden group"
          >
            <div className="absolute inset-0 bg-cyan-400/20 opacity-0 group-hover:opacity-100 transition-opacity blur-xl" />
            <span className="text-3xl z-10 transition-transform group-hover:scale-110">✨</span>
          </motion.button>

          <button onClick={() => navigate('/history')} className="flex-1 py-3 text-[10px] font-black text-white/40 uppercase text-center active:scale-90 transition-all tracking-widest">History</button>
        </div>
      </div>
